// bg_slidemove_prone.c -- part of bg_pmove functionality
...
Arnout: extended SlideMove for prone
==================
*/
...
qboolean PM_SlideMoveProne( qboolean gravity ) {
	...
	for ( bumpcount=0 ; bumpcount < numbumps ; bumpcount++ ) {
		...
		if (trace.fraction > 0) {
			// body is fine, now check our legs
			...
			org[0] = trace.endpos[0] + flatforward[0] * -32;
			org[1] = trace.endpos[1] + flatforward[1] * -32;
			//org[2] = trace.endpos[2] + 12.f;	// 12 units to play with
			...
			pm->trace( &pronetrace, org, playerlegsProneMins, playerlegsProneMaxs, point, pm->ps->clientNum, pm->tracemask );

			if( pronetrace.startsolid ) {
				// entity is completely trapped in another solid
				pm->ps->velocity[2] = 0;	// don't build up falling damage, but allow sideways acceleration
				pm->pmext->proneGroundTime = pm->cmd.serverTime;
				return qtrue;
			...
			if( pronetrace.fraction < 1.f ) {
				VectorCopy( pronetrace.endpos, org );
				//VectorSet( point, org[0], org[1], org[2] + 9.6f );
				VectorSet( point, org[0], org[1], org[2] + ( 24.f - 2.4f ) );

				pm->trace( &pronetrace, org, playerlegsProneMins, playerlegsProneMaxs, point, pm->ps->clientNum, pm->tracemask );

				if( !pronetrace.allsolid ) {
					// actually covered some distance
					VectorCopy (trace.endpos, pm->ps->origin);

					pm->pmext->proneLegsOffset = org[2] - trace.endpos[2];
				} else {
					pm->ps->velocity[2] = 0;	// don't build up falling damage, but allow sideways acceleration
//					pm->pmext->proneGroundTime = pm->cmd.serverTime;
					return qtrue;
...
Arnout: extended StepSlideMove for prone
==================
*/
...
//	vec3_t		delta, delta2;
	...
	if( PM_SlideMoveProne( gravity ) == 0 ) {
