...
qboolean Bot_ScriptInitBot( int entnum )
{
	...
	//
	bs = &botstates[entnum];
	if (!bs->inuse) return qfalse;
	if (bs->script.data) return qtrue;
	...
	for (i=0; i<numScriptCharacters; i++, bsgd++) {
		if (Q_stricmp( name, bsgd->name ) != 0) continue;
		// check params
		p = bsgd->params;
		//
...
void Bot_ScriptEvent( int entityNum, char *eventStr, char *params )
{
	...
	for (i=0; botScriptEvents[i].eventStr; i++)
	{
		if (!Q_stricmp( eventStr, botScriptEvents[i].eventStr ))
		{	// match found
			eventNum = i;
			break;
		}
...
void Bot_ForceScriptEvent( int entityNum, char *eventStr, char *params )
{
	...
	if (entityNum >= MAX_CLIENTS) return;
